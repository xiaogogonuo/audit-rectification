...
func (rs *ServiceRect) Fetching(ctx context.Context, str string) (model.Response, error) {
	var qv = reflect.ValueOf(rs)
	for name, pattern := range global.Patterns {
		if pattern.MatchString(str) {
			ret := qv.MethodByName(name).Call(
				[]reflect.Value{reflect.ValueOf(ctx), reflect.ValueOf(pattern), reflect.ValueOf(str)})
...
func (rs *ServiceRect) P1(ctx context.Context, reg *regexp.Regexp, str string) (model.Response, error) {
	// TODO: 查询nebula数据库
